...
namespace WeaponRestrict
{
    ...
    public class WeaponRestrictConfig : BasePluginConfig
    {
        ...
        [JsonPropertyName("TeamSizeWeaponLimits")]
        public Dictionary<int, Dictionary<string, int>> TeamSizeWeaponLimits { get; set; } = new Dictionary<int, Dictionary<string, int>>()
{
    { 5, new Dictionary<string, int>() { ["weapon_awp"] = 1 } },
    { 10, new Dictionary<string, int>() { ["weapon_awp"] = 2 } }
};
    ...
    public class WeaponRestrictPlugin : BasePlugin, IPluginConfig<WeaponRestrictConfig>
    {
        ...
        public Dictionary<int, Dictionary<string, int>> TeamSizeWeaponLimits = new();
        ...
        public void OnConfigParsed(WeaponRestrictConfig newConfig)
        {
            ...
            newConfig.TeamSizeWeaponLimits ??= new();

            TeamSizeWeaponLimits = newConfig.TeamSizeWeaponLimits;
        ...
        public HookResult OnWeaponCanAcquire(DynamicHook hook)
        {
            ...
            if (!WeaponQuotas.ContainsKey(vdata.Name) && !WeaponLimits.ContainsKey(vdata.Name) && !TeamSizeWeaponLimits.Any(kvp => teamSize >= kvp.Key && kvp.Value.ContainsKey(vdata.Name)))
            {
                    return HookResult.Continue;
            }
            ...
            if (client == null || !client.IsValid || !client.PawnIsAlive)
                return HookResult.Continue;
            bool isWeaponLimit = WeaponLimits.ContainsKey(vdata.Name);
            bool isTeamSizeWeaponLimit = TeamSizeWeaponLimits.Any(kvp => teamSize >= kvp.Key && kvp.Value.ContainsKey(vdata.Name));
            bool isWeaponQuota = WeaponQuotas.ContainsKey(vdata.Name);

            if ((isTeamSizeWeaponLimit || isWeaponQuota) && Config.VIPFlag != "" && AdminManager.PlayerHasPermissions(client, Config.VIPFlag))
                return HookResult.Continue;
            ...
            foreach (var teamSizeLimit in TeamSizeWeaponLimits.OrderByDescending(kvp => kvp.Key))
            {
                // Проверяем, подходит ли размер команды под условие
                if (teamSize >= teamSizeLimit.Key)
                {
                    if (teamSizeLimit.Value.TryGetValue(vdata.Name, out int teamSizeSpecificLimit))
                    {
                        limit = teamSizeSpecificLimit;
                        break; // Прерываем цикл, так как нашли подходящее правило
                    }
                }
            }

            //string message2 = $"afterCurrent limit: {limit}";
            //client.PrintToChat(message2);
